...
def detect_events(segments):
    """
    Walk segments chronologically, compare each valid scoreboard
    read against the last valid read to detect events.
    """
    last_valid_sb = None
    last_valid_t  = None
...
def build_highlights(segments):
    """
    Mark a segment as highlight if:
      (a) RMS is in top RMS_PERCENTILE%, OR
      (b) it has a detected event, OR
      (c) it is within CONTEXT_SECS of an event segment
    """
    ...
    event_times = np.array([
        s["start"] for s in segments if s.get("events")
    ])
...
if __name__ == "__main__":
    with open("segments_ocr.json") as f:
    ...
    print(f"\nSaved {len(out):,} highlights → highlights.json")